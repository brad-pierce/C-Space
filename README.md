...
above and in the video is the bundled synthetic demo session (`npm start`, then
...
Node ≥ 22.5. Two commands, one terminal.

```sh
npm install
npm start              # BLOCKS — builds, then serves everything on http://localhost:5199
```

Open <http://localhost:5199>. That is the whole quickstart. `npm start` is a
single process serving the built app, the session tail and the data store on one
port; it builds `dist/` for you first, so there is no separate build step.

On a fresh clone there is no session data and no allowlist, so it boots the
**bundled synthetic demo** (`public/demo/session.json`, generated by `npm run
demo` — invented start to finish, no real transcript in it). Nothing of yours is
on screen and nothing has been read.

**To point it at your own sessions, press `s`.** The setup panel lists the
harnesses it found — Claude Code, Codex, Hermes, OpenClaw — with per-project
session counts. Tick the projects you want, press ingest, and the wall swaps to
your own flagship session while it is still playing. No file to edit, no
restart. Nothing is exposed until you tick it; see
[Privacy](#privacy-the-project-allowlist) for what that means and what ingesting
writes to disk.

<details>
<summary>CLI equivalents, and the two-process developer setup</summary>

The panel is a front-end for the same allowlist the CLI has always used, so you
...
npm run build-library  # parse the allowed sessions into the data store
```

For UI work you want Vite's hot reload, which `npm start` does not have because
it serves a build. That path is **two blocking processes in two terminals**:

```sh
npm run dev            # BLOCKS — Vite + HMR on http://localhost:5199
npm run live           # BLOCKS — session tail on http://localhost:5198
```

**Run both, or live mode silently does nothing.** `npm run dev` has no tail of
its own, so with only the first one running the library's LIVE TAIL row stays
empty and no live session ever appears — the app is working correctly, it simply
has no tail to talk to. `npm start` avoids this entirely, which is why it is the
quickstart. Also note `npm run live` reads the allowlist once at startup, so
restart it after editing that file by hand.
...
```sh
npm install
npm start                # then press `s` for the setup panel — tick projects, ingest
```

Or by hand, if you would rather not use the panel:
